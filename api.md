...
data node for distributing data across the cluster. Note that existing
...
* an attempt is made to run the function inside a transaction.
...
| `password`           | Password to be used on the remote data node. The default is to not use a password. |
...
| `database`          | Name of database used on the remote data node     |
| `node_created`      | Was the node created                              |
...
want to use the `time` column as the time partitioning column and the
`location` column as the space partitioning column. You also want to
distribute the chunks of the hypertable on two data nodes
`dn1.example.com` and `dn2.example.com`.

To do this, you can just type:
...
it. For more information, see [`detach_data_node`](#detach_data_node).
